...
用gnuplot画sar数据。
需要注意的是 `set timefmt`指定的是读取数据文件的格式，而 `set format x`指定的是x轴显示的格式。如果不指定`set format`，类似`00:10:01`这样的数据会默认显示成`10:00`[2]。
...
![gnuplot sar](http://www.annhe.net/wp-content/uploads/2015/08/sa01.png)
